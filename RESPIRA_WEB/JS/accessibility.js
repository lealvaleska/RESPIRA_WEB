...
(function() {
  ...
  function initModoAcolhedorMain() {
    if (elements.modoAcolhedorMain) {
      ...
      if (modoAcolhedorSaved === 'true') {
        elements.modoAcolhedorMain.checked = true;
      ...
      elements.modoAcolhedorMain.addEventListener('change', () => {
        ...
        // Sincronizar com toggle do header
  ...
  function updateToggleStatus(isEnabled) {
    if (elements.toggleStatus) {
      ...
      elements.toggleStatus.style.color = isEnabled ? 'var(--sage-green-dark)' : 'var(--text-muted)';
    }
  }
  ...
  function updateSpacing() {
    ...
    // Atualizar preview
    if (elements.previewBox) {
      elements.previewBox.classList.remove('spacing-normal', 'spacing-medium', 'spacing-large');
